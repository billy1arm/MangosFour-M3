...
#if defined (MISTS)
//    uint32 spellDifficultyId = info->GetSpellDifficulty();
    m_spellInfo = info;
#else
...
#elif defined (MISTS)
    uint32 spellSpeed = m_spellInfo->GetSpeed();
    uint32 triggeredBySpellSpeed = m_triggeredBySpellInfo->GetSpeed();
#endif
...
#elif defined (MISTS)
        uint32 spellIcon = m_spellInfo->GetSpellIconID();
#endif
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    uint32 spellIcon = m_spellInfo->GetSpellIconID();
    uint32 spellVisual = m_spellInfo->GetSpellVisual(0);
#endif
...
#elif defined (MISTS)
    uint32 powerType = m_spellInfo->GetPowerType();
#endif
...
#elif defined (MISTS)
    uint32 s_powerType = m_spellInfo->GetPowerType();
#endif
...
bool Spell::IsNeedSendToClient() const
{
    ...
    uint32 spellSpeed = m_spellInfo->speed;
#elif defined (MISTS)
    uint32 spellVisual0 = m_spellInfo->GetSpellVisual(0);
    uint32 spellVisual1 = m_spellInfo->GetSpellVisual(1);
    uint32 spellSpeed = m_spellInfo->GetSpeed();
#endif
