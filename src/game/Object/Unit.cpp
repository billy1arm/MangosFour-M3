...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss)
{
    ...
    if (health <= damage)
    {
            ...
            pVictim->GetTypeId() == TYPEID_PLAYER && pVictim->getClass() == CLASS_PRIEST)
        {
            ...
            {
#if defined (CATA)
                uint32 spellProtoIcon = (*itr)->GetSpellProto()->SpellIconID;
#elif defined (MISTS)
                uint32 spellProtoIcon = (*itr)->GetSpellProto()->GetSpellIconID();
#endif
                if (spellProtoIcon == 1654)
                {
...
#if defined (CATA)
            uint32 protoSchoolMask = i_spellProto->SchoolMask;
#elif defined (MISTS)
            uint32 protoSchoolMask = i_spellProto->GetSchoolMask();
#endif
...
void Unit::CalculateDamageAbsorbAndResist(Unit* pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32* absorb, uint32* resist, bool canReflect)
{
...
#if defined (CATA)
                uint32 spellProtoIcon = itr_spellProto->SpellIconID;
#elif defined (MISTS)
                uint32 spellProtoIcon = itr_spellProto->GetSpellIconID();
#endif

                // Frost Warding (chance full absorb)
                if (itr_spellProto->GetSpellFamilyName() == SPELLFAMILY_MAGE && spellProtoIcon == 501)
                {
    ...
    if (incanterAbsorption)
    {
        ...
        for (Unit::AuraList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        {
            SpellEntry const* itr_spellProto = (*itr)->GetSpellProto();
#if defined (CATA)
    uint32 itrSpellProtoIcon  = itr_spellProto->SpellIconID;
#elif defined (MISTS)
    uint32 itrSpellProtoIcon = itr_spellProto->GetSpellIconID();
#endif

            // Incanter's Absorption
            if (itr_spellProto->GetSpellFamilyName() == SPELLFAMILY_GENERIC &&
                itrSpellProtoIcon == 2941)
            {
    ...
    if (preventDeathSpell && RemainingDamage >= (int32)GetHealth())
    {
        switch(preventDeathSpell->GetSpellFamilyName())
        {
            ...
            case SPELLFAMILY_ROGUE:
            {
                // Cheat Death
#if defined (CATA)
                uint32 preventDeathSpellIcon  = preventDeathSpell->SpellIconID;
#elif defined (MISTS)
                uint32 preventDeathSpellIcon = preventDeathSpell->GetSpellIconID();
#endif
                if (preventDeathSpellIcon == 2109)
                {
...
#if defined (CATA)
        uint32 spellIcon  = (*i)->GetSpellProto()->SpellIconID;
#elif defined (MISTS)
        uint32 spellIcon = (*i)->GetSpellProto()->GetSpellIconID();
#endif
...
#elif defined (MISTS)
                    uint32 itrSpellProtoIcon = (*i)->GetSpellProto()->GetSpellIconID();
#endif
                    if (itrSpellProtoIcon == 3263)
                    {
                        DoneTotalMod *= ((*i)->GetModifier()->m_amount + 100.0f) / 100.0f;
                        break;
                    }
...
#elif defined (MISTS)
                        uint32 itrSpellProtoIcon = (*iter)->GetSpellProto()->GetSpellIconID();
#endif
                        if (spellProtoIcon == 1771)
                        {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (pVictim)
            {
                ...
                switch(spellProto->GetSpellFamilyName())
                {
                    ...
                    {
                        // Fire Blast
                        if (spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000002)) && spellProtoIcon == 12)
                        {
                            // Glyph of Fire Blast
                    ...
                    case SPELLFAMILY_DRUID:
                        ...
                        if (spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000004)))
                        {
                            ...
                            if (pVictim->GetAura(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID, UI64LIT(0x000000000000002), 0, GetObjectGuid()))
                            {
                                ...
                                {
#if defined (CATA)
                                    uint32 iterSpellIcon = (*iter)->GetSpellProto()->SpellIconID;
#elif defined (MISTS)
                                    uint32 iterSpellIcon = (*iter)->GetSpellProto()->GetSpellIconID();
#endif
                                    if (iterSpellIcon == 1771)
                                    {
...
uint32 Unit::MeleeDamageBonusDone(Unit* pVictim, uint32 pdamage, WeaponAttackType attType, SpellEntry const* spellProto, DamageEffectType damagetype, uint32 stack)
{
...
#if defined (CATA)
            uint32 protoSchoolMask = spellProto->SchoolMask;
#elif defined (MISTS)
            uint32 protoSchoolMask = spellProto->GetSchoolMask();
#endif
    ...
    uint32 creatureTypeMask = pVictim->GetCreatureTypeMask();
    uint32 schoolMask       = spellProto ? protoSchoolMask : uint32(GetMeleeDamageSchoolMask());
    ...
    if (spellProto)
    {
        ...
        {
#if defined (CATA)
            uint32 itrSpellIcon = (*i)->GetSpellProto()->SpellIconID;
#elif defined (MISTS)
            uint32 itrSpellIcon = (*i)->GetSpellProto()->GetSpellIconID();
#endif
            if (!(*i)->isAffectedOnSpell(spellProto))
            {
                continue;
...
int32 Unit::CalculateAuraDuration(SpellEntry const* spellProto, uint32 effectMask, int32 duration, Unit const* caster, Spell const* spell /*=NULL*/)
{
    ...
    if (caster == this)
    {
        ...
        switch(spellProto->GetSpellFamilyName())
        {
            case SPELLFAMILY_DRUID:
                // Thorns
                if (spellProtoIcon == 53 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000100)))
                {
                    // Glyph of Thorns
            ...
            case SPELLFAMILY_PALADIN:
                // Blessing of Might
                if (spellProtoIcon == 298 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000002)))
                {
                ...
                // Blessing of Wisdom
                else if (spellProtoIcon == 306 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000010000)))
                {
